...
> 문서 버전: v2.5 (AI Insight 콘텐츠 구조·종합 조언·이상치 경고 반영) · 작성일: 2026-09-22
...
| 18 | **확정(v2.5)** | AI Insight 콘텐츠 구조 | 요약·칩·추천액션 3블록 placeholder → **선정 이유·레퍼런스·접촉 채널·준비 타임라인·리스크·정부지원사업 매칭·한계점**을 더한 구조로 확정. 실제 생성 로직/프롬프트는 여전히 M8-a(파이프라인 완성 후)로 보류 (§3.4) |
| 19 | **추가(v2.5)** | AI Insight 리스크 블록 | 규제·물류·대금 등 실행 리스크를 한 섹션에 통합하고 항목별 **심각도(상/중/하)** 표시. 대금 리스크에는 결제 관행(T/T·LC 등) 실무 팁 포함 (§3.4) |
| 20 | **변경(v2.5)** | AI Insight 한계점 블록 | "리스크"와 뒤섞이던 한계점을 **데이터/분석 자체의 한계 1줄**로 축소. 실행 관련 유의사항은 19번 리스크 블록으로 통합 (§3.4) |
| 21 | **변경(v2.5)** | AI Insight 선정 이유 문구 | 점수식·데이터 소스명을 본문에서 빼고 **결과 중심의 쉬운 문장**으로 서술. 원지표·배점 근거는 `더보기` 토글로 이동 (§3.4) |
| 22 | **추가(v2.5)** | 종합 조언 (Portfolio Recommendation) | Top 20 표 상단에 "중소기업이 감당 가능한 1~2개국" 우선순위 추천 배너 추가, 근거 문장 포함 (§3.9) |
| 23 | **추가(v2.5)** | 이상치 경고 배지 | `growth.yoy_pct` / `growth.cagr3_pct`가 비정상적으로 튀는 국가에 규칙 기반(AI 불필요) 경고 배지 표시 (§3.9.1) |
| 24 | **보류·별도 진행(v2.5)** | Trade Show / Expo 일정 | 별도 환경에서 제작 후 최종본에 조립 예정. 확정되면 AI Insight의 "준비 타임라인" 블록이 이 일정 데이터를 소비 (§3.4, §10.2) |
...
- **생성 방식 (여전히 보류)**: AI 프롬프트/LLM 연동은 데이터 파이프라인(M1~M7) 완료 후 진행한다(M8-a). 그 전에는 `pipeline/insight.py`가 고정 문구 또는 빈 상태(`데이터 분석 중…`)를 반환하는 stub이며, 이후 `insight.generate()`만 교체하면 되도록 인터페이스를 미리 고정한다. **하지만 "무엇을 보여줄 것인가"라는 콘텐츠 구조 자체는 아래로 v2.5에서 확정**한다 — 더 이상 임시 placeholder가 아니라 최종 목표 스키마다.

#### 3.4.1 설계 원칙

기존 3블록(요약·칩·추천액션) 구성은 "왜 이 시장인가"와 "위험 요소"는 담지만 "그래서 오늘/이번 주에 뭘 해야 하는가"에 대한 답이 없어 **읽고 덮는 보고서**가 된다는 피드백을 반영한다.

- **결과 중심 문장**: 점수식·정규화 방식·데이터 소스명(예: "UN Comtrade 기준 성장률 점수 82점")을 본문에 그대로 노출하지 않는다. "최근 3년간 이 시장의 한국산 수입은 15%p 늘었지만 경쟁국 대비 점유율은 아직 낮습니다" 처럼 결과 문장으로 풀고, 원지표·배점 breakdown(`score_breakdown`)은 카드 안 `더보기` 토글로 접어 둔다.
- **한계점과 리스크를 분리**한다: "한계점"은 데이터·분석 자체의 한계만 짧게, 진출 시 실행 리스크(규제·물류·대금)는 전부 "리스크" 블록으로 모은다. 두 개념이 섞이면 카드 전체가 "조심하라"는 경고문으로만 읽혀 블루오션 컨셉과 상충된다.
- **리스크는 나열이 아니라 판단 재료로**: 항목마다 심각도(상/중/하)를 붙여 "이건 감당할 만한 수준"이라는 판단이 서게 한다.
- **실행 전환을 돕는 최소 정보**만 추가한다 — 레퍼런스(선례)·접촉 채널·정부지원사업 매칭. 반대로 **"다음 액션 체크리스트"(3~5단계 실행 로드맵)와 "대략적 소요기간·초기비용" 추정은 이번 범위에서 의도적으로 제외**한다. 국가별 우선순위 판단은 §3.9의 종합 조언이 대신하고, 세부 로드맵·비용 추정은 정확도를 보장하기 어려워 신뢰도 리스크가 더 크다고 판단했다.

#### 3.4.2 콘텐츠 블록 (v2.5 확정)

| # | 블록 | 내용 | 톤·표현 규칙 |
|---|---|---|---|
| 1 | 헤더 | 💡 `AI Insight` + `[Live AI Scan]` 배지 + 대상 국가명 | 기존 유지 |
| 2 | 선정 이유 (Why this market) | 2~3문장, 결과 중심 평문. 예: `최근 3년간 이 시장의 한국산 수입은 15%p 늘었지만, 경쟁국 대비 한국 점유율은 아직 낮습니다.` | 점수·산식·소스명은 숨기고 `더보기`로 원지표(`score_breakdown`) 노출 |
| 3 | 핵심 지표 칩 | Export Gap, 전년대비 수입성장률(⬆️/⬇️ + %), 경쟁국 Top3 점유율 합계 | 기존 유지 |
| 4 | 레퍼런스 (선례) | 같은 업종·품목으로 이미 진출한 한국 기업이 있는지, 있다면 진출 방식(직수출/에이전트/현지법인 등) 1~2줄. **없으면 "왜 아직 없는지"** 추정 사유(신시장 형성 초기·규제 장벽·최근 수요 발생 등)까지 제시 | 근거가 약하면 단정 대신 "~로 추정됩니다" |
| 5 | 접촉 채널 | 해당국 담당 **KOTRA 무역관**, 관련 **협회·전시회**, **온라인 B2B 플랫폼**(Alibaba 등 현지 조달 플랫폼 포함) 중 확인 가능한 시작점 2~4개 | 전시회 항목은 별도로 제작 중인 Trade Show/Expo 모듈(§10.2)과 데이터를 공유하되, 그 모듈이 없어도 텍스트만으로 동작해야 한다 |
| 6 | 준비 타임라인 | 5번에서 파악된 **박람회 일정이 있을 때만** 그 일정을 역산해 "언제까지 무엇을 준비해야 하는지" 마일스톤 2~4개 (예: 인증 신청 → 샘플 제작 → 부스 신청 마감) | 박람회 일정이 없으면 이 블록은 렌더링하지 않음(빈 placeholder로 남기지 않음) |
| 7 | 리스크 | 규제·물류·대금 리스크를 한 섹션에 통합, 항목별 **심각도 배지(상/중/하)**. 대금 리스크에는 그 시장의 통상 결제 관행(예: T/T 선불 30%+후불 70%, LC 권장 여부) 실무 팁 포함 | 관세·비관세 장벽(§3.5 C-1)과 내용이 겹치면 요약만 하고 C-1로 링크 |
| 8 | 정부 지원사업 매칭 | 수출바우처·신시장 진출 지원사업 등 이 국가·품목에 매칭될 만한 지원사업 존재 여부 | 사업명·조건의 정확도는 크롤링 신선도에 의존(§10.2) |
| 9 | 한계점 | **데이터·분석 자체의 한계만** 1줄. 예: `관세 데이터는 최근 크롤링 시점 기준이며 실제 통관 시 세율과 다를 수 있습니다.` | 1줄 고정, 실행 리스크는 여기 넣지 않음(7번으로) |
| 10 | 푸터 | `Target: Mexico (멕시코)` · `Predator Engine` | 기존 유지 |

- 갱신 시점: `selectedIso3`가 바뀔 때마다 **위 10개 블록 전체**를 재생성한다 — 요약 문단만이 아니라 칩·레퍼런스·접촉 채널·리스크까지 선택 국가 기준으로 다시 채운다 (`GET /api/country/<iso3>/detail`의 `insight` 필드, §7.3).
- **크롤링 원칙("명확·정확·최신")**: 4·5·6·7(결제 관행)·8번처럼 AI가 외부에서 가져와 채우는 항목은 프롬프트/파이프라인 단에 "**명확·정확·최신**" 3원칙을 고정 지시로 건다 — 모호한 일반론 대신 구체적 출처와 확인 날짜를 남기고, TTL이 지난 정보는 §6.4 캐시 전략과 동일하게 `stale` 플래그를 붙여 노출한다.
...
#### 3.9.0 종합 조언 배너 (Portfolio Recommendation, v2.5 추가)

표 상단, 정렬 드롭다운 아래에 Top 20 전체를 훑고 나서 실제로 어디부터 볼지 정해주는 배너 1개를 둔다.

| 요소 | 명세 |
|---|---|
| 내용 | Top 20 중 **중소기업이 감당 가능한 1~2개국**을 우선순위로 추천 + 그 이유 문장 |
| 판단 기준 | 점수(`score`) 하나가 아니라 **현실적 감당 가능성**까지 함께 고려한다 — 관세·비관세 장벽 수준(`barriers`), 환율 변동성(`fx`), 경쟁 강도(`competitors.top3_share_pct`), 시장 규모 대비 진입 난이도 등. 단순히 "1위 국가라서"가 아니라 **근거를 문장으로 명시** |
| 근거 문장 | 예: `A국은 점수는 3위지만 관세 0%(FTA)·비관세장벽 0건·경쟁 집중도가 낮아, 1위인 B국(비관세장벽 4건·규제 강도 상)보다 중소기업이 먼저 시도하기에 적합합니다.` |
| 신뢰도 표기 | "데이터 분석 결과이며 실제 진출 여부는 종합 판단이 필요합니다" 같은 단서를 짧게 붙여 과신을 방지 |
| 갱신 | `hs:changed`(품목 변경) 시에만 재생성. 국가 클릭(`target:selected`)으로는 바뀌지 않는다(포트폴리오 관점이므로 표 전체를 다시 봐야 함) |
| 생성 방식 | AI Insight(§3.4)와 동일하게 M8-a(파이프라인 완성 후)에서 LLM 연동. 그 전에는 stub이 빈 상태 반환 |
...
**이상치 경고 배지 (v2.5 추가, 규칙 기반 — LLM 불필요)**

`growth.yoy_pct`(1년 성장률) 또는 `growth.cagr3_pct`(3년 CAGR)가 비정상적으로 튀는 국가는 착시(기저효과·소액 기준 급등 등)일 수 있으므로 ⬆️/⬇️ 기호 옆에 ⚠️ 배지를 추가로 표시한다.

| 항목 | 규칙 |
|---|---|
| 판정 기준 | 후보 풀(Stage 2, 상위 50개국) 기준 `yoy_pct`·`cagr3_pct` 각각의 **P5~P95 범위를 벗어나면** 이상치로 판정 (§4.2에서 이미 쓰는 이상치 클리핑 방식과 동일한 사고, 점수 계산과 무관하게 표시만 추가) |
| 표시 | 기호 오른쪽에 작은 ⚠️ + `aria-label="이례적인 증감률 — 기저효과 등 확인 필요"` |
| 팝오버 문구 | 기존 성장률 팝오버(§3.9.1) 하단에 한 줄 추가: `⚠️ 이례적인 수치입니다 (전년도 수입액이 극히 작았을 수 있음 등)` |
| 계산 위치 | `pipeline/scoring.py` 또는 `pipeline/funnel.py`에서 Stage 2 통과 시점에 계산해 `data_flags`에 `growth_outlier_yoy` / `growth_outlier_cagr3`로 추가 (§7.3) — **AI Insight 생성과 무관하게 항상 동작** |
...
```json
{
  ...
  "portfolio_advice": {
    "recommended_iso3": ["MEX"],
    "reason": "A국은 점수는 3위지만 관세 0%(FTA)·비관세장벽 0건·경쟁 집중도가 낮아, 중소기업이 먼저 시도하기에 적합합니다.",
    "generated_at": "2026-09-20T09:00:00+09:00"
  }
}
```

- `portfolio_advice`: §3.9.0 종합 조언 배너의 데이터 소스. **품목(hs6) 단위**로 한 번만 생성되며 `target:selected`(국가 클릭)로는 재요청하지 않는다 — `hs:changed`일 때만 갱신. AI Insight(§3.4)와 마찬가지로 실제 생성 로직은 M8-a에서 붙이고 그 전에는 stub이 빈 배열/문구를 반환한다.
...
- `data_flags`에는 기존 `fx_fallback_yfinance` / `fx_missing` / `tariff_missing` 외에 v2.5부터 **`growth_outlier_yoy`** / **`growth_outlier_cagr3`**(§3.9.1 이상치 경고)가 추가될 수 있다.

**`/api/country/<iso3>/detail` (요약, `insight`는 v2.5 콘텐츠 구조 §3.4.2 반영)**
...
```json
{
  ...
  "insight": {
    ...
    "why_market": "…",
    "chips": [],
    "reference": { "exists": true, "examples": ["…"], "reason_if_none": null },
    "contacts": [ { "type": "kotra|association|expo|b2b_platform", "name": "…", "note": "…" } ],
    "prep_timeline": [ { "milestone": "…", "due": "2027-02-01" } ],
    "risks": [ { "category": "regulation|logistics|payment", "severity": "high|mid|low", "note": "…" } ],
    "gov_programs": [ { "name": "…", "note": "…" } ],
    "limitation": "…",
    "footer": { "target": "…", "engine": "Predator Engine" }
  }
}
```

- `insight`의 `reference`·`contacts`·`gov_programs`·`risks`(대금 리스크의 결제 관행 부분)는 크롤링으로 채우는 항목이라 §3.4.2의 "명확·정확·최신" 원칙을 따른다. `prep_timeline`은 박람회 일정을 못 찾으면 빈 배열이며 프론트는 그 블록을 렌더링하지 않는다.
...
| 9 | Trade Show Schedule | 메인 대시보드 **컴포넌트로는 삭제**(KOTRA 박람회 연동 제거). 다만 **별도 환경에서 독립 모듈로 계속 제작 중** — 완성되면 최종본에 조립하고, AI Insight의 "준비 타임라인"(§3.4.2 #6)이 그 일정 데이터를 소비 | §3.5, §3.4.2, §10.2 |
| 10 | AI Insight | 위치·크기 레이아웃 + **콘텐츠 블록 구조(선정 이유·레퍼런스·접촉 채널·준비 타임라인·리스크·정부지원사업·한계점, §3.4.2)까지 확정.** 실제 LLM 생성 로직·프롬프트 문구만 데이터 파이프라인 완성 후(M8-a) 진행 | §3.4 |
...
| 14 | AI Insight 한계점 vs 리스크 | **한계점 = 데이터/분석 자체의 한계만 1줄.** 규제·물류·대금 등 실행 관련 유의사항은 전부 리스크 블록으로 통합 | §3.4.2 |
| 15 | AI Insight 선정 이유 문구 | 점수식·데이터 소스명 노출 금지, **결과 중심 평문**으로 서술. 원지표·배점 근거는 `더보기` 토글 | §3.4.2 |
| 16 | 리스크 심각도 표기 | 리스크 항목마다 **상/중/하 배지** 부여 (나열로 끝내지 않음) | §3.4.2 |
| 17 | "다음 액션 체크리스트" / 소요기간·비용 추정 | **이번 범위에서 제외.** 전략 우선순위는 §3.9.0 종합 조언이 대신하고, 세부 로드맵·비용 추정은 정확도 리스크가 커서 넣지 않기로 결정 | §3.4.1 |
| 18 | 종합 조언 (Portfolio Recommendation) | Top 20 표 상단 배너, **중소기업 감당 가능 1~2개국** 우선 추천 + 근거. 품목(hs6) 단위로 갱신, 국가 클릭으로는 안 바뀜 | §3.9.0, §7.3 |
| 19 | 이상치 경고 | `growth.yoy_pct`/`cagr3_pct`가 후보 풀 P5~P95를 벗어나면 ⚠️ 배지. **규칙 기반, LLM 불필요** | §3.9.1 |
...
| 4 | **AI Insight 크롤링 대상(레퍼런스·접촉 채널·정부지원사업·결제 관행) 출처 미정** — §3.4.2에서 요구하는 4개 블록은 아직 어떤 사이트/API를 크롤링할지 확정하지 않았고, 관세·환율보다 구조화되지 않은 텍스트라 "명확·정확·최신" 원칙을 지키기 어려울 수 있음 | M8-a 착수 전 블록별 소스 후보를 정하고(KOTRA 사이트, 수출바우처 공고 페이지 등), 소스가 불확실한 블록은 "확인 필요" 문구와 함께 노출하거나 해당 블록을 숨김 |
| 5 | **Trade Show/Expo 모듈 조립 시점 미정** — 별도 환경에서 독립적으로 제작 중이라 이 문서의 M-단계(§8)에는 아직 포함되지 않음. 완성 전까지 AI Insight의 "준비 타임라인" 블록은 항상 빈 상태 | 모듈이 준비되면 §8 개발 순서에 M-단계를 추가하고, 두 모듈이 공유할 데이터 스키마(국가·품목·박람회 일정)를 먼저 맞춰본다 |
